...
static unsigned char ScanSGCmpFlag=0;// 反向补偿标示符
...
volatile char CheckSgnFilter = 0;
volatile char CheckSgnFilterOff = 0;

volatile unsigned char flgSgn = 0;
volatile unsigned char flgSgnOld = 0;
...
static void DrillingHoleProc(long depth);
...
void TongCaoWorkProc(void)
{
    ...
    else if(cTongCaoRunStep == 3)
    {// 等待X轴停机
    	...
    	{
			_lqigangfdelay = dwTickCount + 100;
    	}
    }
    ...
    {// 等待移动到位
        ...
        {
			_lqigangfdelay = dwTickCount + 50;//10 * CustomPlug->XBackDelayTime;
        }
    }
    ...
    {// 减速拉出板尾
		...
		{
			_lqigangfdelay = dwTickCount + 100;//10 * CustomPlug->XBackDelayTime;
		}
    }
...
void DuanCaoWorkPro(void)
{
    ...
    {// 等待扫到板头
    	...
    	{  
        	cDuanCaoRunStep = 100;  
        	_lxbackdelay = dwTickCount + 50;// 至少延时50ms
        	LMV_Dec_Stop(X_AXIS);
    	}
   	}
    ...
    {// X轴 长度走完
        ...
        {
			_lxbackdelay = dwTickCount + 100;//10 * CustomPlug->XBackDelayTime;
        }
    }	
...
void DingHoleScanProc(void) 
{
    ...
    else if(cDingHoleScanStep == 2)
    {
        ...
        if (flgSgn == 0)
        {
            ...
            temp -= HoleinfoF[CheckHoleNum[1] - 1].hole_point / XEncodeDivide;
...
void DingHoleWorkPro(void)
{
    ...
    {// 扫到板头后，赋值要打的孔数
    	...
    	{  
        	...
        	HolePos->Pos_Hole[19] = PluseToPosition(X_AXIS, _gDingHoleDist);
...
uint8 LookForBoardHeadCheck(void)
{
	...
	{
		if(TimeParam->Work2Dist > 20000)
		{
			temp = TimeParam->Work2Dist - 20000;
			if((CurAxisDistance->XDistance < temp))
			{// 超边界
				flag = 1;
			}
		}
	...
	{
		if(TimeParam->Work3Dist > 20000)
		{
			temp = TimeParam->Work3Dist - 20000;
			if((CurAxisDistance->XDistance < temp))
			{// 超边界
				flag = 1;
			}
...
void LookForBoardHead(void)
{
	...
	{
		MV_AccDec_Set(X_AXIS, XIDEACC3, XIDEACC3);
		LMV_Const_Move(X_AXIS, (SpeedParam->IdleSpeed/2), XCCW);// 1);
...
*/
/************************************************/
#pragma interrupt ( Timer1TickISR( vect = VECT( CMT1,CMI1 ), enable ) )
void Timer1TickISR( void )
{// 中断为10us一次的定时器终端
	if(StartFlag > 0)
	{
	    if (CheckObjectSgn) 
...
void ScanCheckHoleProc(void) 
{
	...
	if(cScanHoleStep == 1)
	{
		if(flgSgn > 0)
		{// 检测到板头
			BoardHead = HoleinfoB[0].hole_point;// / XEncodeDivide;
			cScanHoleStep = 2;
		}
	}
	...
	{// 这里只需要检测板尾
		...
		WoodLength = ulEncoderCount - BoardHead;// 
		WoodLength = WoodLength / XEncodeDivide;
		...
		{
			temp = ulEncoderCount;//
			temp = temp - HoleinfoF[CheckHoleNum[1] - 1].hole_point;// / XEncodeDivide;
...
{// 输出扫描数据到调试页面
	...
	for(i = 0;i < (k);i++)
	{
    	temp = HoleinfoF[i].hole_point / XEncodeDivide;
    ...
    {
		Winfos[i - 1].hole_point = HoleinfoB[i].hole_point / XEncodeDivide;;
    }
    j = CheckHoleNum[1] - 1;
    for(i = 0;i < j;i++)
    {
...
unsigned char CheckHoleInfo(void)
{
	...
	endposition = HoleinfoF[CheckHoleNum[1] - 1].hole_point/XEncodeDivide; 
...
void ScanDrillHoleRun(void) 
{
	...
	{// 定时器中断里读取边延数据，这里等待板尾信号
		ScanCheckHoleProc();// 只作扫孔范围判断不对扫孔具体数据处理
		if(cScanHoleStep == 0)
		{// 扫板结束
			bCuiQi=0;
			StartFlag = 0;
	...
	{   	
	    ...
	    {       
			...
			{
				OverLcttmp = Winfos[currentWorkHoleNum].hole_point;
				OverLcttmp -= Winfos[currentWorkHoleNum-1].hole_point;
...
void DrillingHoleProc(long depth)
{
	...
	{	//开始钻孔
	    if(BackDelay < dwTickCount)
	    {
    		...
    		{
    			ScanSGCmpFlag=0; 
    	        OverLcttmp = TempDist.SlowMotorBuchang;
    	        tXAxisStepper.cRealPosi=tXAxisStepper.cRealPosi-OverLcttmp;
    		}
	    	if(DelayTime2 < dwTickCount)
	    	{
	    	    if(SystemMode > 0)
