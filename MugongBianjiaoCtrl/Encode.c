...
static volatile unsigned char PinStatus[3] = {0};
...
static volatile long ulEncodertemp = 0;
static void IOInterrupt(void)
{
    ...
    if(StartFlag > 0)
    {
        ...
        if(PinStatus[2] > 0)
        {// 上升沿
            ulEncodertemp = ulEncoderCount;
            if(CheckHoleNum[0] > 0)
            {
            	ulEncodertemp -= HoleinfoB[CheckHoleNum[0]].hole_point;
				if(ulEncodertemp > 6000)
				{
        ...
        {// 下降沿
            ...
            if(CheckHoleNum[1] > 0)
            {
				ulEncodertemp -= HoleinfoF[CheckHoleNum[1]].hole_point;
				if(ulEncodertemp > 6000)
				{
