...
// X轴根据距离计算脉冲数
unsigned long CalPulseNumber(unsigned short cAxisNo,unsigned long dist)
{
	unsigned long PulseNumber = 0; 
	
	if(dist > 0)
	{
		if(cAxisNo == X_AXIS)
		{// 深度
			PulseNumber = ( long)((double)dist * XGearRatio + 0.5);
		}
		else if(cAxisNo == Y_AXIS)
		{// 位置
			PulseNumber = ( long)((double)dist * YGearRatio + 0.5);
		}
	}
...
// X轴根据脉冲数计算距离
long CalDistance(unsigned short cAxisNo,long pulse)
{
	long dist = 0; 
	...
	//if(pulse > 0)
	{
		if(cAxisNo == X_AXIS)
		{// 深度
			dist = ( long)((double)pulse / XGearRatio + 0.5);
		}
		else if(cAxisNo == Y_AXIS)
		{// 位置
			dist = ( long)((double)pulse / YGearRatio + 0.5);
		}
	}
...
void ReadRobotCurrent(void) 
{
	...
	CurAxisDistance->YDistance = tXAxisStepper.cRealPosi;
	CurAxisDistance->XDistance = ulEncoderCount;// * 2;
	if(ulEncoderCount < 0)
	{
        ulEncoderCount = 0;
	}
