...
void RunInit(void)
{
	...
	//定位孔
	if(_gcLocalHole > 0)// bLocationHole)
	{
...
unsigned char  RunFuncSelect(void)
{
	unsigned char flag = 0;
    ...
    case 100:
    	if(xbackdelay < dwTickCount)
    	{
	    	if(!CheckObjectSgn)
			{
			    ...
			    XWorkOrignPos = temp;//ulEncoderCount / XEncodeDivide;
			    if(flag > 0)
			    {
			    	cRunStep=2;
				}
        ...
        if (!X_DRV)
        {
        	if(_lxbackdelay < dwTickCount)
        	{
	            ...
	            else
	            {
	                ...
	                TempData = TempData - _gcLocalSlot;
