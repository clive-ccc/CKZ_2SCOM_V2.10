...
{
	static unsigned long _lCurDrillRaidus = 0;
	
    switch (cDuanCaoScanStep) 
    {           
    case 1:
        if (flgSgn > 0)// && flgSgnOld == 0)
        {
            cDuanCaoScanStep = 2;  
            TempData = HoleinfoB[0].hole_point / XEncodeDivide;
            BoardEnd = TempData;
    ...
    case 2: 				//下降沿跳变
        if(flgSgn < 1)
        {
            _lCurDrillRaidus = HoleinfoF[CheckHoleNum[1]].hole_point / XEncodeDivide;
            cDuanCaoScanStep = 0;
            //cDuanCaoScanStep = 3;
        }
        WoodLength = ulEncoderCount / XEncodeDivide;
        ...
        break;
    case 3:
    	CheckHoleData.R = ulEncoderCount / XEncodeDivide;
        CheckHoleData.R -= _lCurDrillRaidus;
        if (flgSgn > 0)
        {
            cDuanCaoScanStep = 3;            
        }
