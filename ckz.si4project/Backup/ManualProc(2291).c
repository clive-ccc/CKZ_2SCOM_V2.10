	 ...
	 if((bDotMVST))
	{
		if((!Y_Dotting)&&(!Y_DRV)&&(!(bXRst || bYRst )))
		{
			if(bMVYn)
			{
                ...
                cSetPulse = tYAxisStepper.cRealPosi - TimeParam->DotDis;
                LMV_Pmove(Y_AXIS,cSetPulse);
...
void ManualProc_XAxis(void)
{
	 if((bDotMVST))
	{
		...
		{
			if(bMVXn)//&& (bDrillBackSgn))
			{
			    if((bSystemMode && bDrillBackSgn) || (!bSystemMode && bQiGangBackSgn))
			    {
    				X_Dotting = 1;					
    			      //设定X轴				      
                    cSetPulse = tXAxisStepper.cRealPosi - TimeParam->DotDis;
                    //MoveAction_Pulse(Y_AXIS, cSetPulse, SpeedParam->ManualSpeed);
                    MoveAction_Pulse2(cSetPulse, SpeedParam->ManualSpeed);
                        //MANUAL_SPEED);
    				bMVXn=0;
				}
			}
			else if(bMVXp)//&&bDrillBackSgn)
			{
			    if((bSystemMode && bDrillBackSgn) || (!bSystemMode && bQiGangBackSgn))
			    {
    				X_Dotting = 1;					
    			      //设定X轴
    				cSetPulse = tXAxisStepper.cRealPosi + TimeParam->DotDis;
    				//MoveAction_Pulse2(Y_AXIS, cSetPulse, SpeedParam->ManualSpeed);	
                    MoveAction_Pulse2(cSetPulse, SpeedParam->ManualSpeed);
    				bMVXp=0;
				}
			}			
	...
	else
	{
		...
		if(X_Dotting && 
		    (!(bMVXn ||bMVXp)||(bMVXn &&X_ZERO)||bMVXp&&(tXAxisStepper.cRealPosi > tXAxisStepper.cMaxLen)) )		
		{
			bMVXn =0;
			bMVXp=0;
			X_Dotting = 0;
...
{  
    if(tYAxisStepper.cRstStep == 1)
    {
        if(Y_ZERO)
        {
            ...
            LMV_Const_Move(Y_AXIS,3,0);// 远离原点
        }
        else
        {
    ...
    else if(tYAxisStepper.cRstStep == 2)
    {
        if(!Y_DRV)
        {
            ...
            MV_AccDec_Set(Y_AXIS,100,100);
            LMV_Const_Move(Y_AXIS,3,1);// 向原点走
        }
    }
...
{       
    ...
    if(tXAxisStepper.cRstStep == 1)
    {
        if(X_ZERO)
        {
            ...
            LMV_Const_Move(X_AXIS,1,0);// 远离原点
        }
        else
        {
    ...
    else if(tXAxisStepper.cRstStep == 2 && !X_DRV)
    {
        ...
        MV_AccDec_Set(X_AXIS,150,150);
        //LMV_Const_Move(Y_AXIS,SpeedParam->ManualSpeed,1);// 向原点走
        LMV_Const_Move(X_AXIS,speed,1);// 向原点走
    }
